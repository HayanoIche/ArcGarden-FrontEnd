      ...
      <div className="flex flex-row flex-wrap justify-center gap-3 lg:gap-4 w-full">
        {items.map((item) => (
          <InfoCard
            ...
            title={item.title}
            text={item.text}
            className="flex flex-col items-center justify-center text-center bg-[var(--g1)] rounded-xl p-6 w-[calc(50%-0.5rem)] md:w-[calc(50%-0.5rem)] lg:w-64 min-h-40"
            titleClassName="font-bold text-[var(--text-bold)] text-xl"
            textClassName="font-bold text-[var(--text-main)] text-sm mt-3 "
          />
        ))}
      </div>
    </div>
